...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      try {
        ...
        console.error("Failed to fetch products from Turso:", e);
      }
  ...
  const fetchAnalytics = async () => {
    // Mock analytics for now or fetch from Supabase if table exists
    setAnalytics({
      viewsByDay: [],
      topPages: [],
      topReferrers: [],
  ...
  const fetchRecentActivity = async () => {
    try {
      ...
      try {
        latestProducts = await db.select({
